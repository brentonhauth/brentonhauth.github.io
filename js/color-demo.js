(function() {
    ...
    $("input.R2G").on("input", function() {
        ...
        let fc = "rgba("+clr[0]+","+clr[1]+","+clr[2]+","+0.67+")";
    ...
    $("input.rgbMixer").on("input", function() {
        ...
        if ($("#UseHSL").val() === "1") {
            var mixc = ColorHSL.mix(c1, c2, bias/100, false); mixc.a = 0.67;
            $(".hslMixer").get(0).style.backgroundColor = mixc.formatHSL;
        }

        mix = ColorRGB.mix(c1, c2, bias/100, false); mix.a = 0.67;
    ...
    $("button.rgbMixer").click(function() {
        UseHSL.value = UseHSL.value == 1 ? 0 : 1;
        if (UseHSL.value == 1) {
            $(this).text("Hide HSL Mix");
            $("div.hslMixer").show("slow");
